...
class HmcCliConnection:
    ...
    def execute(self, cmd):
        ...
        # This env 'SSH_PRIVTAE_KEY_FILE' takes the input of ssh private key file with absolute path
        if os.environ.get('SSH_PRIVATE_KEY_FILE'):
            ssh_private_key = os.environ.get('SSH_PRIVATE_KEY_FILE')
        ...
        if self.pwd:
            ...
            ssh_hmc_cmd = "ssh -i '{0}' '{1}'@{2} {3} '{4}'".format(ssh_private_key, self.user, self.ip, host_key_ignore, cmd)
        else:
